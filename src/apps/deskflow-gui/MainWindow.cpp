...
void MainWindow::secureSocket(bool secureSocket)
{
  ...
  if (secureSocket) {
    ...
    ui->lblConnectionSecurityStatus->setPixmap(QIcon::fromTheme("security-high").pixmap(QSize(32, 32)));
  } else {
    ui->lblConnectionSecurityStatus->setToolTip(tr("Insecure Connection"));
    ui->lblConnectionSecurityStatus->setPixmap(QIcon::fromTheme("security-low").pixmap(QSize(32, 32)));
  }
}
